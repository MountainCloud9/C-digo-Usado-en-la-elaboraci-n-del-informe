...
class CalculadoraFolinCiocalteu:
    ...
    def __init__(self):
        self.num_puntos = 6
        self.volumen_total = 1000  # mL (volumen final de cada punto)
    ...
    def generar_tabla_resultados(self, concentraciones, volumenes, conc_ac_galico):
        ...
        print(f"📏 Volumen final por punto: {self.volumen_total} μL")
...
3. PROCEDIMIENTO DE REACCIÓN:
   a) Agregar a cada punto:
      • 100 μL de Reactivo Folin-Ciocalteu (diluido 1:10 si es necesario)
      • Incubar 5 minutos a temperatura ambiente
   
   b) Agregar:
      • 100 μL de Na2CO3 ({conc_na2co3})
...
""")
    ...
    def guardar_resultados(self, concentraciones, volumenes, conc_ac_galico, conc_na2co3):
        ...
        with open(nombre_archivo, 'w', encoding='utf-8') as f:
            ...
            f.write(f"  • Volumen final por punto: {self.volumen_total} μL\n\n")
            
            f.write("TABLA DE VOLÚMENES:\n")
            f.write("-"*70 + "\n")
            f.write(f"{'Punto':<8} {'Conc. (mg/L)':<15} {'Ác. Gálico (μL)':<20} {'Agua (μL)':<20}\n")
            f.write("-"*70 + "\n")
            
            for i, (conc, vol) in enumerate(zip(concentraciones, volumenes), 1):
                vol_agua = self.volumen_total - vol
                f.write(f"{i:<8} {conc:<15.2f} {vol:<20.2f} {vol_agua:<20.2f}\n")
